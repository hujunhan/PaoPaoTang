...
void drawall();
PIMAGE grass, fire, p, playerbehind, playerfront, playerleft, playerright, tree, wall,win,lose, start,wait;
int online();
int offline();
void offlineMove();
...
int online()
{
	...
	char* smap;
	smap = *map;
	send(sClient, smap, sizeof(map), 0);
	while (1)
	{
		ret = recv(sClient, revData, 255, 0);
		if (revData[1] != 0)
		{
			PlayerMove1(revData[0]);
		}
